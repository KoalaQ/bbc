...
function showPages(name) { //初始化属性
	...
	this.argBid = 'bid'; //参数名
	this.bid="";
	this.argOrder = 'order'; //参数名
	this.order="";
	this.argType = 'type'; //参数名
	this.type=1;
...
showPages.prototype.getBid = function(){ //丛url获得当前页数,如果变量重复只获取最后一个
	var args = location.search;
	var reg = new RegExp('[\?&]?' + this.argBid + '=([^&]*)[&$]?', 'gi');
	var chk = args.match(reg);
	this.bid = RegExp.$1;
}
showPages.prototype.getOrder = function(){ //丛url获得当前页数,如果变量重复只获取最后一个
	var args = location.search;
	var reg = new RegExp('[\?&]?' + this.argOrder + '=([^&]*)[&$]?', 'gi');
	var chk = args.match(reg);
	this.order = RegExp.$1;
}
showPages.prototype.getType = function(){ //丛url获得当前页数,如果变量重复只获取最后一个
	var args = location.search;
	var reg = new RegExp('[\?&]?' + this.argType + '=([^&]*)[&$]?', 'gi');
	var chk = args.match(reg);
	this.type = RegExp.$1;
}
...
showPages.prototype.createUrl = function (page) { //生成页面跳转url
	...
	var args = location.search;
	var serachs=args.substring(this.argName.length+1+this.page.length+1, args.length);
	var reg = new RegExp('([\?&]?)' + this.argName + '=[^&]*[&$]?', 'gi');
...
showPages.prototype.printHtml = function(mode){ //显示html代码
	...
	this.getBid();
	this.getType();
	//document.getElementById("url").innerHTML=this.createUrl(1)+","+location.search+","+location.protocol + '//' + location.host + location.pathname;
	this.checkPages();
	this.showTimes += 1;
